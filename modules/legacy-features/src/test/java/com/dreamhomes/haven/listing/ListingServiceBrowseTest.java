...
class ListingServiceBrowseTest {
    ...
    void browsePublicAsksRepositoryForOnlyLiveListings() {
        ...
        when(propertyApi.findSummariesByIds(Set.of(7L))).thenReturn(Map.of(7L, summaryAt(7L)));
    ...
    void browsePublicAttachesParentPropertyToEachListing() {
        ...
        when(propertyApi.findSummariesByIds(Set.of(7L, 8L)))
    ...
    @Test
    void emptyPageDoesNotHitPropertyApi() {
        Pageable page = PageRequest.of(0, 20);
        when(listingRepository.findByStatus(eq(ListingStatus.LIVE), eq(page)))
        ...
        verify(propertyApi, never()).findSummariesByIds(any());
    }
