 ...
 * <ul>
 *   <li>{@link ListingApi} — status check + ownership.</li>
 *   <li>{@link OfferApi} — accepted-offer eligibility for review participation.</li>
...
public class ReviewService implements ReviewApi {
    ...
    public ListingReview post(Long reviewerId, Long listingId, Long revieweeId,
        ...
        if (reviewerIsOwner) {
            if (!offerApi.hadAcceptedOffer(listingId, revieweeId)) {
                throw new InvalidRevieweeException();
            }
        } else {
            if (!offerApi.hadAcceptedOffer(listingId, reviewerId)) {
