...
public class ReviewController {
    ...
    public ReviewResponse post(
        ...
        return ReviewResponse.from(reviewService.post(
    ...
    public Page<ReviewResponse> listForListing(
        ...
        return reviewService.listForListing(listingId, pageable).map(ReviewResponse::from);
    }
    ...
    public Page<ReviewResponse> listForUser(
            ...
            @RequestParam(defaultValue = "20") int size) {
        Pageable pageable = PageRequest.of(Math.max(page, 0), Math.min(Math.max(size, 1), MAX_PAGE_SIZE));
        return reviewService.listForReviewee(userId, pageable).map(ReviewResponse::from);
    }
